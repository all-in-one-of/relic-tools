...
class CheckoutContext:
	def __init__(self, parent, name, folder, asset_folder):
...
class CheckoutDialog(QDialog):
	def __init__(self, parent=maya_main_window()):
		...
		self.contexts = [
			CheckoutContext(self, 'Model', os.environ['ASSETS_DIR'], 'model')
		]
	...
	def new_asset(self):
		text, ok = QInputDialog.getText(self, 'New Asset', 'Enter name (ie: relic_pillar_01)')
		if ok:
			filename = str(text.replace(' ', '_'))
			amu.createNewAssetFolders(self.context.folder, filename)
			self.context.add_item(filename)
			self.refresh()
		return
